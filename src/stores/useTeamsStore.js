...
export const useTeamsStore = create((set, get) => ({
  ...
  addTeam: async ({ name, manager, players, logo, color }) => {
  ...
  updateTeam: async (id, { name, manager, players, logo, color, group }) => {
    try {
      const cleanName = (name || '').trim()
      const cleanManager = (manager || '').trim()
      const normalizedPlayers = normalizePlayers(players)
      ...
      await teamsService.updateTeamDoc(id, {
        ...
        players: normalizedPlayers,
        logo: logo || null,
        color: logo ? null : (color ?? undefined),
      })
        ...
        teams: state.teams.map((team) =>
          team.id === id
            ? {
                ...
                players: normalizedPlayers,
                logo: logo ?? team.logo,
                color: logo ? null : (color ?? team.color),
                group: group || null,
              }
            : team
        ),
