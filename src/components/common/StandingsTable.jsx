...
export default function StandingsTable({ standings, bestThirdTeamIds = new Set() }) {
      ...
      <table className="w-full text-sm text-start">
        ...
        <tbody>
          {standings.map((team, index) => {
            ...
            const isDirectQualify = index < 2
            const isQualifyingBestThird = bestThirdTeamIds.has(team.id)

            return (
              <tr
                key={team.id}
                className={`border-t border-border hover:bg-bg-surface transition-colors
                  ${isDirectQualify ? 'border-s-2 border-s-success bg-success/5' : ''}
                  ${isQualifyingBestThird ? 'border-s-2 border-s-warning bg-warning/5' : ''}
                `}
              >
