...
export default function MatchRow({ match, teamA, teamB, onClick }) {
  ...
  return (
    <DarkCard
      ...
      onClick={onClick}
      className={`p-4 border-t border-border relative ${isFinal ? 'ring-2 ring-accent/50 ring-offset-2 ring-offset-bg-primary' : ''}`}
    >
      {isFinal && (
        <div className={`absolute top-2 ${isAr ? 'left-2' : 'right-2'} bg-gradient-to-br from-accent to-accent-hover text-black dark:text-white px-2 py-1 rounded-full flex items-center gap-1 shadow-lg shadow-accent/30 z-10`}>
      ...
      <div className="flex items-center justify-between text-xs text-text-secondary mb-3">
        ...
        <span>{match.round ? (t[lang].round ? t[lang].round[match.round] : match.round) : t[lang].group(match.group)}</span>
      </div>
